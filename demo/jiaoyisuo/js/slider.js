...
function showTab(event) {
	...
	if(action != 'no'){
		...
		if(action == 'next'){
			direction = direction - (426 * (index - current));
			tab_d = tab_d - (71 * (index - current));
		}else{
			direction = direction + (426 * (current - index));
			tab_d = tab_d + (71 * (current - index));
		}
